...
echo "==============================iptables configure complete============================="
#Only support CentOS system

# 获取SSH端口,执行成功返回值为 0,否则返回非0值
...
# 限制80端口单个IP的最大连接数为10
$IPT -I INPUT -p tcp --dport 80 -j connlimit --connlimit-above 10 -j DROP

# 允许80(hhtp)/873(RSYNC)/443(HTTPS)/20,21(FTP)/25(SMTP) 端口连接
...
$IPT -A INPUT -p tcp -m tcp --dport $sshport -j ACCEPT
